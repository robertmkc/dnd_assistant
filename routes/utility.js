...
function NamedAPIResourceWithCategory(data) {
    return{
        count: data.length,
        results: data.map((item) => {
            var cat2 = "";
            if (item.vehicle_category != null) cat2 = item.vehicle_category;
            if (item.gear_category != null) cat2 = item.gear_category;
            if (item.weapon_category != null) cat2 = item.weapon_category;
            if (item.armor_category != null) cat2 = item.armor_category;
            if (item.tool_category != null) cat2 = item.tool_category;
            return {
                name: item.name,
                url: item.url,
                cat: item.equipment_category,
                cat2: cat2
            }
        })
      }
...
function NamedAPIResourceWithChallengeRating(data) {
    return{
        count: data.length,
        results: data.map((item) => {
            return {
...
function NamedAPIResourceWithLevel(data) {
    return{
        count: data.length,
        results: data.map((item) => {
            return {
...
var utility = {
    ...
    NamedAPIResourceWithCategory,
    NamedAPIResourceWithChallengeRating,
    NamedAPIResourceWithLevel,
    ClassAPIResource,
    upperFirst,
    subrace_map,
    proficiency_map,
